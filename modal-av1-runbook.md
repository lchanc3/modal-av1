...
16 核編碼設 nonpreemptible）要 3 倍費率，一小時的片約 $7.5。
...
每個階段跑完會回報自己的用量（elapsed × 核心數 × 費率），driver 彙總後存進
job，完成的工作在歷史裡會顯示「實際 $X　預估 $Y　±Z%　·　容器 N 核時」，
被搶佔重跑過也會標出次數。

**核心秒只含成功跑完的容器**（被搶佔中途死掉的不會回報），但**費用對得上帳單**
—— 費率本身已經把那段攤進去了，見「成本速查」。2026-09-19 的 62 分鐘全片：
回報 $2.50、帳單 $2.50。

送件表單的估算是刻意保守不起來的粗估：

- 片長寫死 1 小時（server 不知道實際片長，要知道就得開容器跑 ffprobe）
- 速度常數 `0.5 × (核心/16) × 1.35` 是從 16 核 preset 3 推的，crf 越低越慢，沒反映
- 沒算切段、合併、driver

費率校正之後，這個估算的量級是對的（實測 $1.31 估 / $2.50 收是**校正前**的數字，
校正後同樣的送件會估到 $2.6 左右）。但片長一旦不是 1 小時就會整個歪掉，
所以**實際數字仍以完成後的那一行為準**。
...
**自動補掃邊界**（預設開啟）：階梯步進 2 會跳過真正的臨界點。掃完之後如果
「過門檻中體積最小的那個」和「比它大的第一個不過的」之間還有整數，就自動把
中間補跑完。實測 E1001：28/30/32 掃出建議 30，補掃 31 之後發現 31 也過、
而且比 30 小 7.7%，建議因此改成 31。多跑一兩個 crf 只要幾分錢，
換算到全片可能是上百 MB。
...
預設費率（$0.0000262/core/s）買的是**可搶佔**實例，隨時可能被回收。
...
8 核 + 8 GiB = 8 × 0.0000262 + 8 × 0.00000276 = 每秒 $0.000232，即 **$0.83/小時**。
16 核 + 16 GiB 則是 **$1.67/小時**。
`nonpreemptible=True` 是 3 倍費率，只值得用在 driver 那種 0.25 核的小容器上。

這兩個費率是**從帳單回歸出來的**，不是 Modal 的牌價 —— 它們把容器開機、縮容
閒置、被搶佔重跑這些「函式內量不到的時間」一起吸收進來了，所以
`elapsed × 核心數 × 費率` 直接對得上帳單。詳見 `encode.py` 的 `RATE_CPU_S`。
